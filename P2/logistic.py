...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for iteration in range(0, max_iterations):
        ...
        dw = np.dot(X.T, (A - y))
        db = np.sum(A - y)
        w = w - step_size * dw * (1 / N)
        b = b - step_size * db * (1 / N)
...
def multinomial_train(X, y, C,
    ...
    TODO: add your code here
    """
    y = np.int_(np.eye(C)[y].T)

    for iteration in range(0, max_iterations):
        A = np.dot(w, X.T) + b[:, np.newaxis]

        A_max = np.max(A, axis=0)
        A_max = np.reshape(A_max, (1, A_max.shape[0]))
        A = np.subtract(A, A_max)

        A_exp = np.exp(A)
        A_exp_sum = np.sum(A_exp, axis=0)
        A_exp_sum = np.reshape(A_exp_sum, (1, N))

        A = np.divide(A_exp, A_exp_sum)
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for c in range(0, C):
        y_class = copy.deepcopy(y)
        for index in range(0, len(y_class)):
            if y_class[index] == c:
                y_class[index] = 1
            else:
                y_class[index] = 0

        w_class, b_class = binary_train(X, y_class, w[c], b[c])
        w[c] = w_class
        b[c] = b_class
...
def OVR_predict(X, w, b):
    ...
    preds = np.argmax(sigmoid(np.dot(w, X.T) + b[:, np.newaxis]), axis=0)
    assert preds.shape == (N,)
    return preds
