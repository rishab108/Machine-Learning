...
class max_pool:
    ...
    def forward(self, X):
        ...
        max_cols, self.argmax_cols = dnn_im2col.maxpool_im2col_indices(X, self.max_len, self.max_len, self.padding,
                                                                       self.stride)
    ...
    def backward(self, X, grad):
        out_backward = dnn_im2col.maxpool_col2im_indices(grad, self.argmax_cols, X.shape, self.max_len, self.max_len,
...
class softmax_cross_entropy:
    ...
    def forward(self, X, Y):
        ...
        forward_output = - np.sum(np.multiply(self.expand_Y, self.calib_logit - np.log(self.sum_exp_calib_logit))) / \
                         X.shape[0]
        return forward_output
