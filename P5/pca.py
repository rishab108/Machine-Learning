...
def pca(X=np.array([]), no_dims=50):
    ...
    eigenvalues_sorted = np.argsort(eigenvalues, kind='quicksort', order=None)
    eigenvalues_top = eigenvalues_sorted[:no_dims]
    M = eigenvectors[:, eigenvalues_top]
    Y = np.matmul(X, M)
...
def reconstruction_error(orig=np.array([]), decompressed=np.array([])):
    ...
    error = np.sum(np.subtract(orig, decompressed) ** 2) / len(orig)
    return error
