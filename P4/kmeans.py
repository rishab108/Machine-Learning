...
class KMeans():
    ...
    def fit(self, x):
        ...
        while max_iter < self.max_iter:
            ...
            means_square = np.sum(means ** 2, axis=1).reshape(means.shape[0], 1)
            x_mean_product = -2 * (np.dot(x, means.T))
            dists = x_square + means_square.T + x_mean_product
            membership = np.argmin(dists, axis=1)
            J_new = np.sum(dists[np.arange(N), membership]) / N
            ...
            for k in range(self.n_cluster):
                temp = np.where(membership == k)
                k_entries = x[temp]
                means[k] = np.mean(k_entries, axis=0)

            max_iter += 1

        return (means, membership, max_iter)
...
class KMeansClassifier():
    ...
    def fit(self, x, y):
        ...
        for k in range(self.n_cluster):
            temp = np.where(membership == k)
            labels_for_k = y[temp]
            centroid_labels[k] = np.argmax(np.bincount(labels_for_k))
    ...
    def predict(self, x):
        ...
        x_square = np.sum(x ** 2, axis=1).reshape(x.shape[0], 1)
        means_square = np.sum(centroids ** 2, axis=1).reshape(centroids.shape[0], 1)
        x_mean_product = -2 * (np.dot(x, centroids.T))
        dists = x_square + means_square.T + x_mean_product
        membership = np.argmin(dists, axis=1)
