...
def transform_image(image, code_vectors):
    ...
    x_square = np.sum(image ** 2, axis=1).reshape(image.shape[0], 1)
    means_square = np.sum(code_vectors ** 2, axis=1).reshape(code_vectors.shape[0], 1)
    x_mean_product = -2 * (np.dot(image, code_vectors.T))
...
def kmeans_classification():
    ...
    np.savez('results/k_means_classification.npz',
             y_hat_test=y_hat_test, y_test=y_test, centroids=classifier.centroids,
