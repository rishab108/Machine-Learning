...
def linear_regression_noreg(X, y):
    ...
    # W = (X.T X)-1 X.T Y closed-form notation
...
def regularized_linear_regression(X, y, lambd):
    ...
    # W = (X.T X + lambda I)-1 X.T Y closed-form notation
    identity = lambd * np.identity(X.shape[1])
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
    ...
    for lambd in lambds:
        ...
        rss = np.sum((yval - np.dot(Xval, w)) ** 2, axis=0)
        if rss < min_rss:
            min_rss = rss
            bestlambda = lambd
...
def test_error(w, X, y):
    ...
    # mse = (Y - W.X)^2 / N
    err = np.sum((y - np.dot(X, w)) ** 2, axis=0) / X.shape[0]
    return err
