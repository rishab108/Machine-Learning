...
def compute_distances(Xtrain, X):
    ...
    # (x - xtrain)^2 = x^2 + xtrain^2 - 2 (x*xtrain)

    X_square = np.sum(X ** 2, axis=1).reshape(X.shape[0], 1)
    Xtrain_square = np.sum(Xtrain ** 2, axis=1).reshape(Xtrain.shape[0], 1)
    X_Xtrain_product = -2 * (np.dot(X, Xtrain.T))
    dists = X_square + Xtrain_square.T + X_Xtrain_product
...
def predict_labels(k, ytrain, dists):
    ...
    sorted_dists = np.argsort(dists, axis=1, kind='quicksort', order=None)
    top_k_sorted_dists = sorted_dists[:, :k]
    test_size = top_k_sorted_dists.shape[0]

    for i in range(test_size):
        for j in range(k):
            top_k_sorted_dists[i][j] = ytrain[top_k_sorted_dists[i][j]]

    ypred = np.zeros(test_size, dtype=int)
    for i in range(0, test_size):
        ypred[i] = np.argmax(np.bincount(top_k_sorted_dists[i]))
...
def compute_accuracy(y, ypred):
    ...
    total_lables = ypred.shape[0]
    correct_label = 0
    for i in range(total_lables):
        if ypred[i] == y[i]:
            correct_label += 1

    acc = correct_label / total_lables
    return acc
